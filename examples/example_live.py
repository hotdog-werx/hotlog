...
def main():
    ...
    parser = argparse.ArgumentParser(description="Live logging example using hotlog")
    parser.add_argument(
        "-v",
        "--verbose",
        action="count",
    ...
    with live_logging("Installing dependencies...") as live:
        for i in range(1, 6):
            ...
            if i == 3:
                ...
                live.error("Package installation failed", package=f"package-{i}", error="checksum mismatch")
            else:
